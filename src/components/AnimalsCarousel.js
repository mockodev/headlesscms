...
function AnimalsCarousel({ animal }) {
  ...
  const next = () => {
    if (animating) return;
    const nextIndex = activeIndex === animal.thumbnails.length - 1 ? 0 : activeIndex + 1;
    setActiveIndex(nextIndex);
  };

  const previous = () => {
    if (animating) return;
    const nextIndex = activeIndex === 0 ? animal.thumbnails.length - 1 : activeIndex - 1;
    setActiveIndex(nextIndex);
  };
